...
public:
    ...
    void write(const int16_t* samples, uint32_t nFrames);
    void close();
    bool isOpen() const { return file_ != nullptr; }
    uint64_t bytes() const { return dataBytes_; }
    double seconds() const { return rate_ ? static_cast<double>(dataBytes_) / (rate_ * channels_ * 2) : 0.0; }
