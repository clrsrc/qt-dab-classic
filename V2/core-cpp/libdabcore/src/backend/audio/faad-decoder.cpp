// DAB Classic v3: portiert aus Qt-DAB sources/backend/audio/faad-decoder.cpp
// (Jan van Katwijk, GPLv2+), siehe faad-decoder.h.
#
/*
...
#include	<cstdio>
	...
	faadDecoder::faadDecoder        (PcmSink s): sink (std::move (s)) {
...
uint8_t channels;
	...
	if (channels == 2) {
	   pcmBuffer. resize (samples / 2);
	   for (int i = 0; i < samples / 2; i ++)
	      pcmBuffer [i] = complex16 (outBuffer [2 * i], outBuffer [2 * i + 1]);
	   if (sink)
	      sink (pcmBuffer. data (), samples / 2, (int)sampleRate,
	                            hInfo. ps != 0, hInfo. sbr != 0);
	}
	else
	if (channels == 1) {
	   pcmBuffer. resize (samples);
	   for (int i = 0; i < samples; i ++)
	      pcmBuffer [i] = complex16 (outBuffer [i], outBuffer [i]);
	   if (sink)
	      sink (pcmBuffer. data (), samples, (int)sampleRate,
	                            hInfo. ps != 0, hInfo. sbr != 0);
	}
	else
