// DAB Classic v3: portiert aus Qt-DAB sources/frontend/fic-handling/fic-handler.cpp (Jan van Katwijk, GPLv2+), Qt entfernt: QObject/Signale -> ReceiverCallbacks, QString -> std::string (UTF-8), QMutex -> std::mutex; FIG-Auswertung unveraendert.
#
/*
...
/**
  ...
  */
		...
		ficHandler::ficHandler (ReceiverCallbacks *cb,
	                                uint8_t dabMode,
	                                uint8_t cpuSupport):
	                                    fibDecoder (cb),
	                                    params (dabMode),
...
//	and compute the errors
	...
	if (ficBlocks >= 40) {	// 4 blocks per frame, app 10 frames per sec
	   emitCb (cb -> ficBer, (float)ficErrors / ficBits);
	   ficBlocks	 = 0;
	   ficErrors	 /= 2;
	   ficBits	 /= 2;
	}
	...
	for (int i = ficno * 3; i < ficno * 3 + 3; i ++) {
	   ...
	   if (!check_CRC_bits (ficBlock, 256)) {
	      ...
	      if (successRatio > 0)
	         successRatio --;	
	      if (fibCounter == 0)
	         emitCb (cb -> ficQuality, successRatio, 100 / 50);
	      continue;
	   }
	   ...
	   if (successRatio < RANGE)
	      successRatio ++;
	   if (fibCounter == 0)
	      emitCb (cb -> ficQuality, successRatio, 100 / RANGE);
...
void	ficHandler::startFICDump	(const std::string &fileName) {
	ficDumpPointer. store (fopen (fileName. c_str (), "w + b"));
}
