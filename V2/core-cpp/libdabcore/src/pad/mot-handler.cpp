// DAB Classic v3: portiert aus Qt-DAB sources/backend/data/mot/mot-handler.cpp
// (Jan van Katwijk, GPLv2+), siehe mot-handler.h. Die in v1 globale
// motTable ist jetzt Member (mehrere Paketdienste gleichzeitig).
#
/*
...
//
	...
	motHandler::motHandler (BackendCallbacks *cb,
	                        uint32_t	SId) {
	this	-> cb		= cb;
	this	-> SId		= SId;
	orderNumber		= 0;
...
int32_t	i;
	...
	uint32_t segmentSize    = ((motVector [0] & 0x1F) << 8) |
	                                motVector [1];
	if (segmentSize + 2 > motVector. size ())
	   return;
	switch (groupType) {
	   case 3:	// mot header
	      if (segmentNumber == 0) {
	   ...
	   case 6:
	      if (segmentNumber == 0) { 	// MOT directory
	         ...
	         theDirectory	= new motDirectory (cb,
	                                            SId,
	                                            transportId,
	                                            segmentSize,
	                                            dirSize,
