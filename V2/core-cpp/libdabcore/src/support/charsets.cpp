// DAB Classic v3: portiert aus Qt-DAB sources/support/charsets.cpp
// (Jan van Katwijk / Przemyslaw Wegrzyn, GPLv2+), Qt entfernt:
// EBU-Latin -> UCS2-Tabelle unveraendert, Ausgabe als UTF-8 std::string.
// Die UCS2-Variante verhaelt sich wie v1 (nur das niederwertige Byte
// jedes 16-Bit-Zeichens wird uebernommen).
#
/*
...
uint16_t i;
	...
	switch (charset) {
	   case UnicodeUcs2:
//	      wie v1: QChar (buffer [2 * i + 1]) - nur das zweite Byte
	      for (int i = 0; i < size / 2; i ++) {
	         appendUtf8 (s, (uint8_t)(buffer [2 * i + 1]));
	      }
	      break;

	   case UnicodeUtf8:
//	      v1: QString::fromUtf8 - Bytes sind bereits UTF-8
	   ...
	   default:
	      s. clear ();
	      for (i = 0; i < length; i++) {
	         appendUtf8 (s, ebuLatinToUcs2 [((uint8_t*) buffer)[i]]);
	      }
	}
