// DAB Classic v3: portiert aus Qt-DAB sources/support/equalizer.h (Jan van Katwijk, GPLv2+), unveraendert uebernommen.
#
/*
...
Complex	equalize	(Complex v) {
	...
	if (I_avg < 0) I_avg = 0;
	if (Q_avg < 0) Q_avg = 0;
	DABFLOAT denom	= std::sqrt (I_avg) * std::sqrt (Q_avg);
	if (!std::isfinite (denom) || denom < 1.0e-10f)
	   return v;
	DABFLOAT Beta	= -IQ_avg / denom;
	DABFLOAT K	= v_q + Beta * v_i;
	Q_out		= compute_avg (Q_out, square (K), Alpha_);
	if (Q_out < 0) Q_out = 0;
	if (!std::isfinite (Q_out) || Q_out < 1.0e-10f)
	   return v;
	DABFLOAT Q_gain	= std::sqrt (I_avg / Q_out);
	if (!std::isfinite (Q_gain))
	   return v;
