...
fn print_event(t: Duration, ev: &Event) {
    ...
    match ev {
        ...
        Event::EwsAlert { phase, sub_ch, stage, stage_raw, iid, locations, is_test, relevant } => println!("{ts}  EWS  {phase:?} subCh={sub_ch} stage={stage} (roh 0x{stage_raw:02X}) iid={iid} test={is_test} relevant={relevant:?} {} Orte", locations.len()),
