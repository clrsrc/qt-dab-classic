...
| `set_volume` | `percent` |
| `set_mute` | `muted` |
| `set_audio_device` | `id?` – Kennung aus `audio_devices`; fehlend/leer = Standardgerät des Systems (folgt dessen Wechseln). Unbekannte `id` bleibt gemerkt, bis das Gerät angesteckt wird; solange spielt der Standard (Log `warn`) |
| `refresh_audio_devices` | Geräteliste neu einlesen (Pa_Terminate/Pa_Initialize, kurze Lücke), danach `audio_devices` |
...
| `set_tpeg` | `enabled` (Standard an). Wie `set_epg` für den TPEG-Paketdienst des Ensembles (FIG 0/13 Appl-Type 4, DSCTy 5 TDC mit Datengruppen, ETSI TS 103 551; in NRW „ARD TPEG“ auf 11D und 9A, nicht im Bundesmux 5C): der Kern startet ihn selbst als Background-Slot, sobald die FIC ihn meldet, und liefert die Datengruppen als `tdc_group`; `enabled: false` beendet nur die vom Kern gestarteten TPEG-Dienste. `state.tpeg_enabled`. dabcored: `--no-tpeg` |
...
| `ensemble_found` | `eid`, `name`, `channel`, `ecc` (Extended Country Code aus FIG 0/9, 0 = noch unbekannt; additiv 17.09.2026, für RadioDNS) |
| `ensemble_ecc` | `ecc` – nachgereicht, wenn FIG 0/9 erst nach dem Ensemble-Namen kommt (nur bei Änderung, Reset bei Kanalwechsel; additiv 17.09.2026) |
...
| `tdc_group` | `sid`, `group_type`, `data_b64` – Nutzdaten einer MSC-Datengruppe eines TDC-Paketdienstes (DSCTy 5 mit Datengruppen, TS 101 759 4.1.2); Paket-CRC, FEC und – falls gesetzt – Datengruppen-CRC hat der Kern geprüft, Kopf/Session-Header sind entfernt. Bei TPEG (TS 103 551): ein oder mehrere komplette TPEG-Transportrahmen (`FF 0F`, Länge, Kopf-CRC, Typ, Body), ggf. Null-Padding; `group_type` = Datengruppentyp (TPEG: 0). ARD sendet ~7,8 KB alle ~5 s, Datengruppen-CRC-Flag 0. Die App dekodiert daraus TEC-Verkehrsmeldungen (`dab-app::tpeg`: SFW/zlib, SNI, TEC 3.2 mit MMC, OpenLR- und TMC-Ortsreferenz); TFP wird nur erkannt. Diagnose im Kern: `DABCORE_TDC_DUMP=<datei>` schreibt jede Gruppe roh als `[u16 BE Länge][Bytes]` |
...
| `audio_devices` | `devices[]` (`id`, `name`, `is_default`), `current?` (`id` des spielenden Geräts) – nach `ready` (als drittes Ereignis, nach der Viterbi-Log-Zeile), nach `get_state` (direkt nach `state_snapshot`), nach `set_audio_device`/`refresh_audio_devices` und von selbst, wenn der Kern eine Änderung der Geräte bemerkt (Windows: Wächter alle 2 s über IMMDeviceEnumerator); mit `--no-audio` leere Liste und `current: null`. Windows: nur WASAPI-Geräte (je Gerät einmal), `id` = WASAPI-Endpoint-ID `{0.0.0.00000000}.{guid}`, stabil über Sitzungen und Umstecken; sonst `<hostapi>:<name>` |
| `ews_present` | |
