...
//
	...
	errorCode = (hackrf_error) (hackrf_set_sample_rate (theDevice,
	                                   (float)(SAMPLERATE * OVERSAMPLE_FACTOR)));
	if (errorCode != HACKRF_SUCCESS) {
	   delete library_p;
	   throw (device_exception (hackrf_error_name (errorCode)));
	}
	...
	errorCode = (hackrf_error)
	            (hackrf_baseband_filter (theDevice, 1536000));
	if (errorCode != HACKRF_SUCCESS) {
	   delete library_p;
	   throw (device_exception (hackrf_error_name (errorCode)));
	}
...
void	hackrfHandler::handle_VGAGain	(int newGain) {
	if ((newGain <= 62) && (newGain >= 0)) {
	   ...
	   vgagainDisplay	-> display (newGain & ~0x01);
	}
}
...
hackrf_error errorCode;
float correctedRate = (SAMPLERATE + correction) * OVERSAMPLE_FACTOR;

	if (correctedRate < 2000000.0f || correctedRate > 20000000.0f)
	   return;
	errorCode = (hackrf_error)hackrf_set_sample_rate (theDevice,
	                                                  correctedRate);
	if (errorCode != HACKRF_SUCCESS) {
...
// the filter must be set after the sample rate to be not overwritten
	int bw = std::min (1536000, (int)(correctedRate * 0.75f));
	hackrf_baseband_filter (theDevice, bw);
	if (errorCode != HACKRF_SUCCESS) {
	   statusLabel -> setText (hackrf_error_name (errorCode));
...
int	nrSamples	= transfer -> valid_length / 2;
	...
	for (int i = 0; i + 1 < nrSamples; i += OVERSAMPLE_FACTOR) {
	   ...
	   buffer [bufferIndex]	= std::complex<int8_t> ((int8_t)(re / 2),
	                                                (int8_t)(im / 2));
	   bufferIndex ++;
	}
	if (ctx -> toSkip > 0)
...
QString freqS		= QString::number (freq);
	...
	for (int w = 0; w < 100 && vgaGainSlider -> value () != vgaValue; w++)
	   usleep (1000);
	...
	for (int w = 0; w < 100 && lnaGainSlider -> value () != lnaValue; w++)
	   usleep (1000);
...
void	hackrfHandler::setAmpEnable	(bool b) {
	AmpEnableButton	-> setChecked (b);
	handle_Ampli (0);
}

bool	hackrfHandler::getAmpEnable	() {
	return AmpEnableButton -> isChecked ();
}

void	hackrfHandler::adjustGain	(float snr) {
	if (!running. load ())
	   return;
int	currentVga	= vgaGainSlider -> value ();
int	newVga		= currentVga;

	if (snr < 8.0f && currentVga < 58)
	   newVga = currentVga + 2;
	else
	if (snr > 18.0f && currentVga > 6)
	   newVga = currentVga - 2;

	if (newVga != currentVga) {
	   vgaGainSlider -> setValue (newVga);
	   handle_VGAGain (newVga);
	}
}
