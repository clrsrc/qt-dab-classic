...
//	of "reset"
	...
	scanAmpRetried		= false;
	scanOriginalAmpState	= false;
	running. store (true);
}
...
//	ofdm handler.
//	If scanning, try toggling the amp before giving up on the channel,
//	since strong signals (e.g. 5C national mux) can overload with amp on.
void	RadioInterface::no_signal_found () {
	if (theSCANHandler. active () && !scanAmpRetried) {
	   scanAmpRetried	= true;
	   scanOriginalAmpState	= theDeviceHandler -> getAmpEnable ();
	   theDeviceHandler	-> setAmpEnable (!scanOriginalAmpState);
	   return;
	}
	channel_timeOut ();
}
...
//
	   ...
	   if (!muteTimer. isActive ()) {
	      if (!theAudioPlayer -> is_QtAudio () && audioVolume < 100) {
	         float gain = (float)audioVolume / 100.0f;
...
deviceHandler	*inputDevice = theDeviceChooser.
	...
	QString gm = value_s (theQSettings, DAB_GENERAL,
	                      GUI_MODE, GUI_MODE_CLASSIC);
	if (gm != GUI_MODE_WINAMP)
...
void	RadioInterface::localSelect_SS (const QString &service,
	...
	if (audioDumping) {
	   fprintf (stderr, "service switch blocked: recording active\n");
	   return;
	}
...
void	RadioInterface::channel_timeOut () {
	...
	if (!theSCANHandler. active ())
	   return;
//
//	If we toggled the amp for a retry, restore the original
//	amp state unless the toggle actually helped find services
	if (scanAmpRetried && channel. nrServices == 0) {
	   theDeviceHandler -> setAmpEnable (scanOriginalAmpState);
	}
	scanAmpRetried = false;
...
//	In Winamp mode, skip custom button colors - let QSS handle it
	QString guiMode = value_s (theQSettings, DAB_GENERAL,
	                           GUI_MODE, GUI_MODE_CLASSIC);
...
QPixmap p;
	...
	if (running. load () && theDeviceHandler != nullptr)
	   QMetaObject::invokeMethod (theDeviceHandler,
	      [this, snr]() { theDeviceHandler -> adjustGain (snr); },
	      Qt::QueuedConnection);
	if (!theNewDisplay. isHidden ())
...
void	RadioInterface::setVolume	(int n) {
	...
	if (n == 0)
	   ...
	   if (channel. audioActive)
	      setSoundLabel (true);
	   if (theAudioPlayer != nullptr && theAudioPlayer -> is_QtAudio ())
	      static_cast<Qt_Audio *>(theAudioPlayer) -> setVolume (n);
...
void	RadioInterface::startJournaline		(int currentKey) {
	if (journalineHandler != nullptr)
	   return;
	// suppress journaline window in winamp mode
	QString guiMode = value_s (theQSettings, DAB_GENERAL,
	                           GUI_MODE, GUI_MODE_CLASSIC);
	if (guiMode == GUI_MODE_WINAMP)
	   return;
