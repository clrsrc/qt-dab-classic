 ...
 */
	...
	skin    = skin == "Combinear"     ? ":res/skins/Combinear.qss" :
	          ...
	          skin == "WinampClassic" ? ":res/skins/WinampClassic.qss" : "";
...
//	here we go for real!!
	...
	dabSettings.beginGroup (DAB_GENERAL);
	QString guiMode = dabSettings.value (GUI_MODE,
	                                     GUI_MODE_CLASSIC).toString ();
	dabSettings.endGroup ();
	fprintf (stderr, "GUI MODE: '%s'\n", guiMode.toUtf8 ().data ());
	WinampShell *winampShell = nullptr;
	if (guiMode == GUI_MODE_WINAMP) {
	   fprintf (stderr, "WINAMP MODE: creating shell\n");
	   // suppress all sub-window visibility in winamp mode
	   dabSettings.beginGroup (DAB_GENERAL);
	   dabSettings.setValue (CONFIG_WIDGET_VISIBLE, 0);
	   dabSettings.setValue (DEVICE_WIDGET_VISIBLE, 0);
	   dabSettings.setValue (SNR_WIDGET_VISIBLE, 0);
	   dabSettings.setValue (TECHDATA_VISIBLE, 0);
	   dabSettings.setValue (NEW_DISPLAY_VISIBLE, 0);
	   dabSettings.endGroup ();
	   dabSettings.sync ();

	   // hide RadioInterface from taskbar but keep it alive for backend
	   myRadioInterface->setWindowFlags (
	      myRadioInterface->windowFlags () | Qt::Tool);
	   myRadioInterface->move (-5000, -5000);
	   myRadioInterface->show ();   // must show for backend to work

	   // hide any windows that RadioInterface may have opened
	   auto hideChildren = [myRadioInterface] () {
	      for (auto *child : myRadioInterface->findChildren<QWidget *>()) {
	         if (child->isWindow () && child->isVisible () &&
	             child != myRadioInterface) {
	            child->hide ();
	         }
	      }
	   };
	   QTimer::singleShot (500, hideChildren);
	   QTimer::singleShot (2000, hideChildren);
	   QTimer::singleShot (5000, hideChildren);

	   winampShell = new WinampShell (myRadioInterface, &dabSettings);
	   winampShell->show ();
	   winampShell->raise ();
	   fprintf (stderr, "WINAMP MODE: shell shown\n");
	}
	else {
